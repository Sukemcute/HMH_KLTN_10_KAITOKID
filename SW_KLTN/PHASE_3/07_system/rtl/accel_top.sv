`timescale 1ns/1ps
// Top-level accelerator: controller_system + 4 supercluster_wrappers + tensor_dma.
module accel_top (
  input  logic        clk,
  input  logic        rst_n,
  ...
  // PPU parameters (driven by testbench, shared across all SCs/subclusters)
  input  logic signed [31:0] ppu_bias    [32],
  input  logic signed [31:0] ppu_m_int   [32],
  input  logic        [5:0]  ppu_shift   [32],
  ...
  // Read channel (AR captures addr; RDATA held until RREADY — see mmio_rdata latch below)
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      mmio_addr_rd   <= '0;
      s_axil_rvalid  <= 1'b0;
  ...
  // CSR decode uses correct addr on AR handshake (registered mmio_addr_rd is one cycle late)
  wire [11:0] mmio_addr_mux = mmio_we ? mmio_addr_wr
                           : ((s_axil_arvalid && s_axil_arready) ? s_axil_araddr : mmio_addr_rd);
  // RDATA must stay valid while RVALID (TB samples after AR); latch at end of AR cycle
  ...
  // Controller AXI read signals (for descriptor fetch)
  logic [39:0] ctrl_axi_araddr;
  logic [7:0]  ctrl_axi_arlen;
  logic        ctrl_axi_arvalid, ctrl_axi_arready_i;
  logic        ctrl_axi_rready;
  // After AR handshake ARVALID may drop before RLAST — still belong to controller read burst
  logic        ctrl_rd_busy;
  ...
  // AR handshake: use m_axi_arready (slave) — not ctrl_axi_arready_i (avoids comb loop with mux below)
  ...
  // ─── 4× SuperCluster write merge: broadcast tile → 4 dma_wr can assert together.
  // Without per-master ready/bvalid gating, all SCs see global AW/W/B handshakes → deadlock.
  ...
  // ─── 4× SuperCluster read merge (AR + R) — same class of bug as write path
  logic [1:0] axir_grant;
  logic       axir_busy;
  logic [1:0] axir_pick;
  logic       ar_idle_handshake;
  logic [3:0] sc_ar_first;
  // tensor_dma drives m_axi_rready only in RD_DATA; in RD_AR it is 0. accel_top must not
  // override that with 1'b1 or R beats complete while the DMA FSM is still in RD_AR → desync/hang.
  ...
  generate
    for (gi = 0; gi < 4; gi++) begin : gen_sc
      supercluster_wrapper #(
        ...
        // Mirror W-channel fix: axir_busy registers on the AR-handshake cycle; TB can assert
        // RVALID in that same cycle while this mask still sees axir_busy==0 → RD_DATA never
        // sees beats, prefill never completes, tiles never reach SWIZZLE_STORE (AW audit stays 0).
        ...
        // Allow first W beat in the same cycle as AW handshake: axiw_busy is a registered
        // flag, so without the aw_idle_handshake term tensor_dma can enter WR_DATA while
        // this mux still sees axiw_busy==0 → W never handshakes, DMA deadlocks, AW never
        // reaches the TB slave (COSIM_DMA_AUDIT AW total=0).
  ...
  // ───── AXI Arbiter (read: ctrl burst > SC round-robin; write: SC serialized) ─────
  always_comb begin
    ...
    // Read channel mux:
    // - While controller owns an outstanding read burst, RREADY follows the fetch engine.
    // - If controller is only waiting on AR (arv=1, ctrl_rd_busy=0), do NOT tie m_axi_rready
    //   to ctrl_axi_rready=0: an R beat can still be valid on the bus (e.g. tail of a prior SC
    //   burst). Starving RREADY deadlocks ARREADY on many slaves/models.
    // Same-cycle AR grant uses controller path even if ctrl_rd_busy is still 0 this comb cycle
    if (ctrl_rd_busy) begin
      ...
      // SC-owned beats must use that SC's tensor_dma rready; else drain unowned rvalid.
      ...
      // SuperCluster DMA read — follow each SC's tensor_dma rready when it owns AR/R.
      if (axir_busy) begin
        m_axi_araddr  = sc_araddr[axir_grant];
        m_axi_arlen   = sc_arlen[axir_grant];
        m_axi_arvalid = sc_arvalid[axir_grant];
    ...
    // SC write: one transaction at a time (see axiw_busy / axiw_grant)
    if (axiw_busy) begin
