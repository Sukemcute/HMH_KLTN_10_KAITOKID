...
def align_and_concat(
    ...
    Bước 1: Xác định common domain qua compute_common_scale()
...
def align_and_add(
    ...
    Bước 1: Align cả hai nhánh về common domain
    Bước 2: Add với saturation INT16 trung gian
    Bước 3: Requant về output scale (nếu khác common scale)
    ...
    common_scale, common_zp = compute_common_scale(
        [scale_A, scale_B], [zp_A, zp_B], strategy=strategy
    )

    # Align về common domain
    A_common = requant_to_common(A_int8, scale_A, zp_A, common_scale, common_zp)
    B_common = requant_to_common(B_int8, scale_B, zp_B, common_scale, common_zp)

    # Add với saturation (INT16 intermediate để tránh overflow)
    ...
    # Đưa về output range
    if scale_out is None or (abs(scale_out - common_scale) < 1e-12 and zp_out == common_zp):
        # Output scale = common scale: chỉ clamp + re-add zp_out
        Y_int8 = np.clip(sum_clamped + zp_out, INT8_MIN, INT8_MAX).astype(np.int8)
        scale_out = common_scale
    else:
        # Cần requant về scale_out khác
        sum_float = sum_clamped.astype(np.float64) * common_scale
        Y_int8 = quantize_affine(sum_float, scale_out, zp_out, dtype="int8")
