...
def verify_layer_21():
    print("=== Layer 21 (Concat) Verification (100 Samples, 640x640 Flow) ===")
    ...
    for s in range(100):
        ...
        m = model.model.model[21]
        node = getattr(res, "Layer21")
        ...
        elif 21 in concat_map:
            in_signed = [to_signed_int8(getattr(res, f"Layer{idx}")().int_repr().numpy()) for idx in concat_map[21]]
            in_scale = [float(getattr(res, f"Layer{idx}")().q_scale()) for idx in concat_map[21]]
            in_zp = [int(getattr(res, f"Layer{idx}")().q_zero_point())-128 for idx in concat_map[21]]
        else:
            ref_in = getattr(res, "Layer20")()
            in_signed, in_scale, in_zp = to_signed_int8(ref_in.int_repr().numpy()), float(ref_in.q_scale()), int(ref_in.q_zero_point())-128

        # Run Golden Block
        c_p = {{"scale_out": float(m.fl.scale), "zp_out": int(m.fl.zero_point)-128, "strategy": "offline"}}; my_out, _, _ = block_concat(in_signed, in_scale, in_zp, c_p)
