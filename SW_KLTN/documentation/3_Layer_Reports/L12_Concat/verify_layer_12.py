...
def verify_layer_12():
    print("=== Layer 12 (Concat) Verification (100 Samples, 640x640 Flow) ===")
    ...
    for s in range(100):
        ...
        elif 12 in concat_map:
            in_signed = [to_signed_int8(getattr(res, f"Layer{idx}")().int_repr().numpy()) for idx in concat_map[12]]
            in_scale = [float(getattr(res, f"Layer{idx}")().q_scale()) for idx in concat_map[12]]
            in_zp = [int(getattr(res, f"Layer{idx}")().q_zero_point())-128 for idx in concat_map[12]]
        else:
            ref_in = getattr(res, "Layer11")()
            in_signed, in_scale, in_zp = to_signed_int8(ref_in.int_repr().numpy()), float(ref_in.q_scale()), int(ref_in.q_zero_point())-128
