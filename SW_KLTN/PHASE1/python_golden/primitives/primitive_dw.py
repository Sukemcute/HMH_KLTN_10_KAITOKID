...
def dw_conv_int(
    ...
    """
    Generic Depthwise Convolution INT8.
    Supports any kernel size.
    """
    N, C, Hin, Win = X_int8.shape
    kH, kW = W_int8_per_ch.shape[1], W_int8_per_ch.shape[2]
    ...
    for c in range(C):
        partial_sum_w_c = int(W_i64[c].sum())
        for n in range(N):
            raw_mac = _dw_conv_channel(X_i64[n, c], W_i64[c], stride, padding, zp_x)
            acc_all[n, c] = raw_mac - zp_x * partial_sum_w_c + int(B_int32_per_ch[c])

    Y_int8 = post_process_int32_to_int8(acc_all, M_int_arr, shift_arr, zp_y)
    
    if activation == "relu":
        Y_int8 = apply_relu(Y_int8, zp_y)
