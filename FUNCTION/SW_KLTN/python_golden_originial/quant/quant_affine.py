...
def post_process_int32_to_int8(
    ...
    if acc.ndim == 1:
        # Shape: [Cout]
        for c in range(Cout):
            y_raw[c] = (acc[c] * M_int_arr[c]) >> int(shift_arr[c])
    elif acc.ndim == 4:
        # Shape: [N, Cout, H, W]
        for c in range(Cout):
            y_raw[:, c, :, :] = (acc[:, c, :, :] * M_int_arr[c]) >> int(shift_arr[c])
    ...
    y_int8 = np.clip(y_raw + zp_out, INT8_MIN, INT8_MAX).astype(np.int8)
    return y_int8
...
def build_silu_lut(scale_y: float, zp_y: int) -> np.ndarray:
    ...
    for i in range(256):
        ...
        lut[i] = int(np.clip(round(silu_val / scale_y) + zp_y, INT8_MIN, INT8_MAX))
    return lut
