...
def verify_layer_3():
    print("=== Layer 3 (Conv) Verification (100 Samples, 640x640 Flow) ===")
    ...
    for s in range(100):
        ...
        m = model.model.model[3]
        node = getattr(res, "Layer3")
        ...
        elif 3 in concat_map:
            in_signed = [to_signed_int8(getattr(res, f"Layer{idx}")().int_repr().numpy()) for idx in concat_map[3]]
            in_scale = [float(getattr(res, f"Layer{idx}")().q_scale()) for idx in concat_map[3]]
            in_zp = [int(getattr(res, f"Layer{idx}")().q_zero_point())-128 for idx in concat_map[3]]
        else:
            ref_in = getattr(res, "Layer2")()
            in_signed, in_scale, in_zp = to_signed_int8(ref_in.int_repr().numpy()), float(ref_in.q_scale()), int(ref_in.q_zero_point())-128
