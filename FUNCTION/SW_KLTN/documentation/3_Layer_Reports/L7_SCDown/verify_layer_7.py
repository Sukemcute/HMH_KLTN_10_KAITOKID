...
def verify_layer_7():
    print("=== Layer 7 (SCDown) Verification (100 Samples, 640x640 Flow) ===")
    ...
    for s in range(100):
        ...
        m = model.model.model[7]
        node = getattr(res, "Layer7")
        ...
        elif 7 in concat_map:
            in_signed = [to_signed_int8(getattr(res, f"Layer{idx}")().int_repr().numpy()) for idx in concat_map[7]]
            in_scale = [float(getattr(res, f"Layer{idx}")().q_scale()) for idx in concat_map[7]]
            in_zp = [int(getattr(res, f"Layer{idx}")().q_zero_point())-128 for idx in concat_map[7]]
        else:
            ref_in = getattr(res, "Layer6")()
            in_signed, in_scale, in_zp = to_signed_int8(ref_in.int_repr().numpy()), float(ref_in.q_scale()), int(ref_in.q_zero_point())-128

        # Run Golden Block
        cv1_p = get_conv_params(m.cv1, in_scale, in_zp, is_dw=False); my_out, _, _ = block_scdown(in_signed, cv1_p, get_conv_params(m.cv2, cv1_p["scale_y"], cv1_p["zp_y"], is_dw=True))
