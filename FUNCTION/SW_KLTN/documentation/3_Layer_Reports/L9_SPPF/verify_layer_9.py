...
def verify_layer_9():
    print("=== Layer 9 (SPPF) Verification (100 Samples, 640x640 Flow) ===")
    ...
    for s in range(100):
        ...
        m = model.model.model[9]
        node = getattr(res, "Layer9")
        ...
        elif 9 in concat_map:
            in_signed = [to_signed_int8(getattr(res, f"Layer{idx}")().int_repr().numpy()) for idx in concat_map[9]]
            in_scale = [float(getattr(res, f"Layer{idx}")().q_scale()) for idx in concat_map[9]]
            in_zp = [int(getattr(res, f"Layer{idx}")().q_zero_point())-128 for idx in concat_map[9]]
        else:
            ref_in = getattr(res, "Layer8")()
            in_signed, in_scale, in_zp = to_signed_int8(ref_in.int_repr().numpy()), float(ref_in.q_scale()), int(ref_in.q_zero_point())-128

        # Run Golden Block
        cv1_p = get_conv_params(m.cv1, in_scale, in_zp); c_p = {{"scales": [cv1_p["scale_y"]]*4, "zps": [cv1_p["zp_y"]]*4, "scale_out": cv1_p["scale_y"], "zp_out": cv1_p["zp_y"], "strategy": "offline"}}; my_out, _, _ = block_sppf(in_signed, cv1_p, get_conv_params(m.cv2, cv1_p["scale_y"], cv1_p["zp_y"]), concat_params=c_p)
