...
def verify_layer_17():
    print("=== Layer 17 (Conv) Verification (100 Samples, 640x640 Flow) ===")
    ...
    for s in range(100):
        ...
        m = model.model.model[17]
        node = getattr(res, "Layer17")
        ...
        elif 17 in concat_map:
            in_signed = [to_signed_int8(getattr(res, f"Layer{idx}")().int_repr().numpy()) for idx in concat_map[17]]
            in_scale = [float(getattr(res, f"Layer{idx}")().q_scale()) for idx in concat_map[17]]
            in_zp = [int(getattr(res, f"Layer{idx}")().q_zero_point())-128 for idx in concat_map[17]]
        else:
            ref_in = getattr(res, "Layer16")()
            in_signed, in_scale, in_zp = to_signed_int8(ref_in.int_repr().numpy()), float(ref_in.q_scale()), int(ref_in.q_zero_point())-128

        # Run Golden Block
        is_3x3 = (m.conv.kernel_size[0] == 3); my_out, _, _ = block_conv(in_signed, padding="same", **get_conv_params(m, in_scale, in_zp, is_3x3=is_3x3))
