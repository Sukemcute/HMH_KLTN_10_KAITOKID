...
def verify_layer_10():
    print("=== Layer 10 (QPSA) Verification (100 Samples, 640x640 Flow) ===")
    ...
    for s in range(100):
        ...
        elif 10 in concat_map:
            in_signed = [to_signed_int8(getattr(res, f"Layer{idx}")().int_repr().numpy()) for idx in concat_map[10]]
            in_scale = [float(getattr(res, f"Layer{idx}")().q_scale()) for idx in concat_map[10]]
            in_zp = [int(getattr(res, f"Layer{idx}")().q_zero_point())-128 for idx in concat_map[10]]
        else:
            ref_in = getattr(res, "Layer9")()
            in_signed, in_scale, in_zp = to_signed_int8(ref_in.int_repr().numpy()), float(ref_in.q_scale()), int(ref_in.q_zero_point())-128
